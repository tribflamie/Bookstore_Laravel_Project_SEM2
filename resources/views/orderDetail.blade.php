                                ...
                                @if (session('orderDetails'))
                                    ...
                                    @foreach ($orderDetails as $id => $order)
                                        ...
                                        <tr data-id="{{ $id }}" class="order">
                                            ...
                                            <td><a href="#"
                                                    onclick="window.open('http://localhost:8000/review/{{ $product->id }}', 'newwindow','width=1000,height=1000,left=500,top=300');"
                                                    class="btn btn-success btn-position"><i
                                                        class="fas fa-solid fa-pen"></i></a></td>
                                            <?php else:?>
                                            <td><a href="#" disabled title="This order has not been approved!"
                                                    class="btn btn-warning"><i class="fas fa-solid fa-pen"></i></a></td>
                                            <?php endif;?>
                                        </tr>
                                    @endforeach
                                @else
